<template>
      <el-dialog title="API列表" :visible.sync="isShow" :before-close="cancelClick" @cancel="cancelClick" width="980px">
        <el-table :data="list" style="width: 100%;margin-bottom: 20px;" row-key="id" fit highlight-current-row border >
          ...
          <el-table-column align="center" prop="name" label="API名称" width="200">
            <template slot-scope="scope">
              <p :title="scope.row.name" class="text-ellipsis">{{scope.row.name}}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="url" label="URL" width="200">
            <template slot-scope="scope">
              <p :title="scope.row.url" class="text-ellipsis">{{scope.row.url}}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="maxNumber" label="允许请求数" width="170"/>
          <el-table-column align="center" prop="spaceNumber" label="安全数" width="170"/>
